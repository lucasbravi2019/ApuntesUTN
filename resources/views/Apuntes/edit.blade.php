...
@section('content')
    ...
    <form method="post" action="{{ action('ApuntesController@update', ['carrera' => $carreraSlug, 'materia' => $materiaSlug, 'apuntes' => $apunte->slug]) }}" class="shadow-md max-w-7xl mx-auto p-5 border border-gray-300">
        ...
        <div class="grid grid-cols-2 my-2 shadow p-1 border border-gray-300 max-w-md mx-auto px-10">
            ...
            <input class="border border-gray-300 shadow px-2 py-1 @error('numero_tema') border-4  border-red-500 @enderror" type="number" name="numero_tema" value="{{ $apunte->numero_tema }}">
            @error('numero_tema')
        ...
        <div class="grid grid-cols-4 my-2 shadow p-1 border border-gray-300">
            ...
            <textarea class="border col-span-3 border-gray-300 shadow p-3 @error('desarrollo') border-4 border-red-500 @enderror" name="desarrollo" cols="30" rows="50">{{ $apunte->desarrollo }}</textarea>
            @error('desarrollo')
